...
export class PayService {
  ...
  async pay (userId: number, orderId: string, payType = 'wxpay') {
    ...
    try {
      ...
      if (order.payPlatform === 'pockyt') {
        return this.payPockyt(userId, orderId, payType)
      }
    } catch (error) {
  ...
  /* Pockyt支付 */
  async payPockyt (userId: number, orderId: string, payType = 'wxpay') {
    // query order
    const order = await this.orderEntity.findOne({ where: { userId, orderId } })
    if (!order) throw new HttpException('订单不存在!', HttpStatus.BAD_REQUEST)
    // query goods
    ...
    if (!goods) throw new HttpException('套餐不存在!', HttpStatus.BAD_REQUEST)
    // assemble params
    const { payPockytPid, payPockytSecret, payPockytNotifyUrl, payPockytReturnUrl, payPockytApiPayUrl } = await this.globalConfigService.getConfigs([
      'payPockytPid',
      'payPockytSecret',
      'payPockytNotifyUrl',
      'payPockytReturnUrl',
    ...
    // const params = {}
    // params['pid'] = Number(payPockytPid)
    // params['type'] = payType
    // params['out_trade_no'] = orderId
    // params['name'] = goods.name
    // params['money'] = order.total
    // params['notify_url'] = payPockytNotifyUrl
    // params['return_url'] = payPockytReturnUrl
    // // params['param'] = 'Mpay';
    // params['sign'] = this.sign(params, payPockytSecret)
    // params['sign_type'] = 'MD5'
    // const queryParams = new URLSearchParams(params).toString()
    // const apiUrl = `${payPockytApiPayUrl}?${queryParams}`

    const now = new Date()
    const utcTime = now.toISOString()
    const { IPN_URL, CALLBACK_URL, REDIRECT_URI_API, MERCHANTNO, STORENO } = process.env
    const params = {
      amount: order.total,
      settleCurrency: 'USD',
      currency: 'USD',
    ...
    const verifySign = this.getVerifySign(params)
    params.verifySign = verifySign

    const {
      data: { ret_code, ret_msg, result },
    } = await axios.post(REDIRECT_URI_API, params)

    if (ret_code !== '000100') {
      throw new HttpException(ret_msg, HttpStatus.BAD_REQUEST)
    }

    return { url_qrcode: null, redirectUrl: result.cashierUrl, channel: payType, isRedirect: true }
  }

  /* Pockyt商户信息查询 */
  async queryPockyt (orderId: string) {
    const { payMpayApiQueryUrl } = await this.globalConfigService.getConfigs(['payMpayPid', 'payMpaySecret', 'payMpayApiQueryUrl'])
    const params = {}
    params['type'] = 2
    params['order_no'] = orderId
