...
<template>
  ...
  >
    ...
    >
      <NScrollbar :size="1">
        <div class="flex h-full flex-col items-center space-y-5">
          <div
            v-for="item in menuList"
            :key="item.menuName"
            class="flex justify-center flex-col items-center button-spacing"
...
.icon-wrapper:hover {
  background-color: #27E093; /* 鼠标滑过时背景变色 */
  padding: 12px; /* 增加内边距，让下方视觉上变粗 */
  filter: drop-shadow(0px 4px 0px rgba(0, 0, 0, 1)); /* 添加不透明的黑色阴影 */
}
.icon-wrapper {
  display: inline-block; /* 使外层容器适应内容 */
  border: 2.5px solid black; /* 黑色边框，保持宽度 */
  border-radius: 14px; /* 圆角 */
  padding: 8px; /* 增加内边距来扩大外边框的大小 */
